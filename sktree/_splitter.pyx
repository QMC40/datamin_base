...
cdef class Splitter:
    ...
    def __cinit__(self, Criterion criterion, SIZE_t max_features,
        ...
        self.split_counter = np.empty(max_features, dtype=np.int32)
    ...
    cdef int init(self,
        ...
        self.s = s
        self.alpha = alpha
        self.initial_split_factor = initial_split_factor
...
cdef class BaseDenseSplitter(Splitter):
    ...
    cdef int init(self,
        ...
        # Call parent init
        Splitter.init(self, X, y, s, alpha, initial_split_factor, sample_weight)
...
cdef class BestSplitter(BaseDenseSplitter):
    ...
    cdef int node_split(self, double impurity, SplitRecord* split,
                        SIZE_t* n_constant_features) nogil except *:
        # HERE! NOTE: only BestSplitter works with categoricals
        """Find the best split on node samples[start:end]
        ...
        cdef double improvement_factor = 1.0
        ...
        while (f_i > n_total_constants and  # Stop early if remaining features
            ...
            if Xf[end - 1] <= Xf[start] + FEATURE_THRESHOLD:
                ...
                n_found_constants += 1
                n_total_constants += 1
                # printf("This one is constant so we bail\n");
                continue
            ...
            if is_cat and True:
                ...
                for i in range(n_present_cats):
                    cats[1][i] = cats[N_SORTS][i/2 + j*(1-i%2)] # magic 
                ...
                for i in range(n_present_cats):
                    cats[2][i] = cats[N_SORTS][i/4 + j*(3-i%4)] # magic 
            ...
            for i in range(N_SORTS): # try all sorts
                ...
                while p < end:
                    ...
                    if self.split_counter[current.feature] == 0:
                        improvement_factor = self.initial_split_factor
                    else:
                        improvement_factor = 1.0
                    
                    # printf("Improvement factor: %f", improvement_factor)

                    current_proxy_improvement = self.criterion.proxy_impurity_improvement()

                    if current_proxy_improvement < 0:
                        current_proxy_improvement = current_proxy_improvement * improvement_factor
                    else:
                        current_proxy_improvement = current_proxy_improvement / improvement_factor
        ...
        if best.pos < end:
            ...
            self.split_counter[best.feature] += 1
            #printf("Split counter for feature %d is now %d, improvement %f \n", best.feature, self.split_counter[best.feature], best.improvement)
