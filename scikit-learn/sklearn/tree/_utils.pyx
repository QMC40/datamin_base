...
def _print_cat_node_split(CategoricalPartition partition, int maxval):
    print(f"present: {partition.present}")
    print(f"side: {partition.side}")
    print_partition(partition, maxval)

def _get_cat_node_split(CategoricalPartition partition, int maxval):
    cdef cnp.ndarray indicator
    indicator = np.zeros(maxval, dtype=np.float64)
    cdef SIZE_t i = 1
    while i <= maxval:
