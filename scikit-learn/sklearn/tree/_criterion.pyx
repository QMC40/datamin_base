...
cdef class Criterion:
    ...
    cdef int init(self, SIZE_t n_features, const BOOL_t[::1] cat_pos, const INT32_t[::1] cat_maxval, const DTYPE_t[:, :]X, const DOUBLE_t[:, ::1] y, const DOUBLE_t[:, :] s, double alpha, 
    ...
    cdef int reset(self) nogil except *:
    ...
    cdef int reverse_reset(self) nogil except *:
    ...
    cdef int update(self, SIZE_t new_pos) nogil except *:
...
cdef class ClassificationCriterion(Criterion):
    ...
    def __cinit__(self, SIZE_t n_outputs,
                  cnp.ndarray[SIZE_t, ndim=1] n_classes, SIZE_t n_sensitive, cnp.ndarray[SIZE_t, ndim=1] n_classes_sens):
        ...
        n_classes_sens : numpy.ndarray, dtype=SIZE_t
            The number of unique classes in each sensitive target   
        """
        ...
        self.n_classes_sens = np.empty(n_sensitive, dtype=np.intp)
        cdef SIZE_t k = 0
        cdef SIZE_t max_n_classes = 0
        cdef SIZE_t max_n_classes_sens = 0
        ...
        for k in range(n_sensitive):
            self.n_classes_sens[k] = n_classes_sens[k]
            # print(f"Class {k}: {self.n_classes_sens[k]} values")
            if n_classes_sens[k] > max_n_classes_sens:
                max_n_classes_sens = n_classes_sens[k]

        max_n_classes_sens = max_n_classes_sens + 1
        self.max_n_classes = max_n_classes
        self.max_n_classes_sens = max_n_classes_sens
        ...
        # Count labels for each output (sensitive) - Used for Fair-Gini
        self.sum_total_sens = np.zeros((n_outputs, max_n_classes_sens), dtype=np.float64) # TODO add max_n_sens
        self.sum_left_sens = np.zeros((n_outputs, max_n_classes_sens), dtype=np.float64)
        self.sum_right_sens = np.zeros((n_outputs, max_n_classes_sens), dtype=np.float64)

        # Count labels for each sens attribute  - Used for Multi-Fair-Gini
        self.sum_total_feat_sens = np.zeros((n_outputs, n_sensitive, max_n_classes_sens), dtype=np.float64)
        self.sum_left_feat_sens  = np.zeros((n_outputs, n_sensitive, max_n_classes_sens), dtype=np.float64)
        self.sum_right_feat_sens = np.zeros((n_outputs, n_sensitive, max_n_classes_sens), dtype=np.float64)

        # print(f" N_outputs: {n_outputs} N_sensitive: {n_sensitive} max_n_classes: {max_n_classes} max_n_classes_sens: {max_n_classes_sens}")
    ...
    cdef int init(self, SIZE_t n_features, const BOOL_t[::1] cat_pos,  const INT32_t[::1] cat_maxval, const DTYPE_t[:, :]X, const DOUBLE_t[:, ::1] y, const DOUBLE_t[:, :] s, double alpha,
        ...
        s : SENSITIVE attribute matrix, shape is S \in R^{num_samples, num_sens_features} - S_ij is the bucket_index for sample i and feature j. S_ij < b_max  
        sample_weight : array-like, dtype=DOUBLE_t
            The weight of each sample
        weighted_n_samples : double
        ...
        # self.n_buckets_attr = n_buckets_attr
        ...
        for k in range(self.n_outputs):
            ...
            memset(&self.sum_total_sens[k, 0], 0, self.max_n_classes_sens * sizeof(double))

            for s_idx in range(self.n_sensitive):    # TODO I suspect this is due to ptr indirection 
                memset(&self.sum_total_feat_sens[k, s_idx, 0], 0, self.max_n_classes_sens * sizeof(double))
        ...
        for p in range(start, end):
            ...
            for k in range(self.n_outputs):
                ...
                c = <SIZE_t> self.s[i, k]   # TODO Note that this now implicitely assumes a single output
                self.sum_total_sens[k, c] += w

                # Add all sensitive attributes
                for s_idx in range(self.n_sensitive):
                    c = <SIZE_t> self.s[i, s_idx]
                    self.sum_total_feat_sens[k, s_idx, c] += w
    ...
    cdef int reset(self) nogil except *:
        ...
        for k in range(self.n_outputs):
            ...
            memset(&self.sum_left_sens[k, 0], 0, self.max_n_classes_sens * sizeof(double))
            memcpy(&self.sum_right_sens[k, 0], &self.sum_total_sens[k, 0], self.max_n_classes_sens * sizeof(double))
            for s in range(self.n_sensitive):    # TODO I suspect this is due to ptr indirection 
                memset(&self.sum_left_feat_sens[k, s, 0], 0, self.max_n_classes_sens * sizeof(double))
                memcpy(&self.sum_right_feat_sens[k, s, 0], &self.sum_total_feat_sens[k, s, 0], self.max_n_classes_sens * sizeof(double))
        return 0
    ...
    cdef int reverse_reset(self) nogil except *:
        ...
        for k in range(self.n_outputs):
            ...
            memset(&self.sum_right_sens[k, 0], 0, self.max_n_classes_sens * sizeof(double))
            memcpy(&self.sum_left_sens[k, 0],  &self.sum_total_sens[k, 0], self.max_n_classes_sens * sizeof(double))
            for s in range(self.n_sensitive):    # TODO I suspect this is due to ptr indirection 
                memset(&self.sum_right_feat_sens[k, s, 0], 0, self.max_n_classes_sens * sizeof(double))
                memcpy(&self.sum_left_feat_sens[k, s, 0], &self.sum_total_feat_sens[k, s, 0], self.max_n_classes_sens * sizeof(double))
        return 0
    ...
    cdef int update(self, SIZE_t new_pos) nogil except *:
        ...
        if (new_pos - pos) <= (end - new_pos):
            for p in range(pos, new_pos):
                ...
                for k in range(self.n_outputs):
                    ...
                    for s_idx in range(self.n_sensitive):
                        c = <SIZE_t> self.s[i, s_idx]
                        self.sum_left_feat_sens[k, s_idx, c] += w
        ...
        for k in range(self.n_outputs):
            ...
            for s_idx in range(self.n_sensitive):
                for b_idx in range(self.n_classes_sens[s_idx]):
                    self.sum_right_feat_sens[k, s_idx, b_idx] = self.sum_total_feat_sens[k, s_idx, b_idx] - self.sum_left_feat_sens[k, s_idx, b_idx]
        self.pos = new_pos
        return 0
...
cdef class FairGini(ClassificationCriterion):
    ...
    The Gini Index is then defined as:
        ...
        index = \sum_{k=0}^{K-1} count_k (1 - count_k)
              = 1 - \sum_{k=0}^{K-1} count_k ** 2
    """
    cdef double _pvt_node_impurity(self, int use_sens) nogil: # hardcoded
        """Evaluate the impurity of the current node.
        ...
        for k in range(self.n_outputs): # only 1
            ...
            for c in range(self.n_classes[k]): # basically [0, 1], 2 classes and 2 sensitives!
                if use_sens == 1:
                    count_k = self.sum_total_sens[k, c]
                else:
                    count_k = self.sum_total[k, c] # n_samples of class c
            ...
            for c in range(self.n_classes[k]):  # basically [0, 1], 2 classes and 2 sensitives!
                ...
                if use_sens == 1:
                    if c == 0:
                        ...
                        prob_left -= GAP
                sq_count_left += prob_left * prob_left 
                ...
                if use_sens == 1:
                    if c == 0:
                        ...
                        prob_right -= GAP
                sq_count_right += prob_right * prob_right 
    ...
    cdef double node_impurity(self) nogil:
        g_label = self._pvt_node_impurity(0) # label
        g_sens = self._pvt_node_impurity(1) # sens
        cdef double res = (1.0 - self.alpha ) * g_label + self.alpha  * (0.5 - g_sens)
        # with gil:
        #     print(f"Res fair {res}")
    ...
    cdef void children_impurity(self, double* impurity_left, double* impurity_right) nogil:
        ...
        cdef double gL_sens
        cdef double gR_sens
        self._pvt_children_impurity(&gL_sens, &gR_sens, 1)

        # with gil:
        #     print("Fair")
        #     print(f"gL_label: {gL_label} gL_sens: {gL_sens}")
        #     print(f"gR_label: {gR_label} gR_sens: {gR_sens}")
...
cdef class MultiFairGini(ClassificationCriterion):
    r"""Multi Attribute Fair Gini Index impurity criterion.
    ...
    cdef double _pvt_node_impurity(self, int use_sens) nogil: # hardcoded
        ...
        if use_sens == 1:
            for k in range(self.n_outputs): # only 1
                interm_gini = 0.0
                for s in range(self.n_sensitive):   # s iterates over sensitive attributes
                    sq_count = 0.0
                    for c in range(self.n_classes_sens[s]):  # TODO c iterates over the buckets feature s 
                        count_k = self.sum_total_feat_sens[k, s, c]
                        prob = count_k / self.weighted_n_node_samples
                        if c == 0:
                            prob += GAP 
                        else:
                        ...
                        sq_count += prob * prob
                        # with gil:
                        #     print(f"Feat {s} Iteration {c} count: {count_k} prob: {prob} weight: {self.weighted_n_node_samples} buckets: {self.n_classes_sens[s]}")
                    # with gil:
                    #     print(f"New: {(1-sq_count)/(2*self.n_classes_sens[s])}")
                    if self.n_classes_sens[s] > 1:
                        interm_gini += (1-sq_count)*(self.n_classes_sens[s]/(2.0*(self.n_classes_sens[s]-1)))  # Normalizes gini from [0,1-1/num_classes]->[0,0.5]
                    
                    # with gil:
                    #     print(f"Feature {s} - interm_gini: {interm_gini/(s+1)}")

                # Normalize over features
                gini += (interm_gini/self.n_sensitive) 
        else:
            for k in range(self.n_outputs): # only 1
                sq_count = 0.0
                for c in range(self.n_classes[k]): # basically [0, 1], 2 classes and s sensitives!
                    count_k = self.sum_total[k, c] # n_samples of class c
                    prob = count_k / self.weighted_n_node_samples
                    sq_count += prob * prob
        ...
        # Normalize over outputs
        # with gil:
        #     print(f"Final Used sens {use_sens} gini: {gini}")
        return gini / self.n_outputs # 1 - sum(p^2)
    ...
    cdef void _pvt_children_impurity(self, double* impurity_left,
        ...
        if use_sens == 1:
            ...
            for k in range(self.n_outputs):
                ...
                for s in range(self.n_sensitive):   # s iterates over sensitive attributes
                    sq_count = 0.0
                    for c in range(self.n_classes_sens[s]):  # c iterates over the buckets feature s 
                        count_k = self.sum_left_feat_sens[k, s, c]
                        prob = count_k / self.weighted_n_left
                        if c == 0:
                            prob += GAP 
                        else:
                            prob -= GAP
                        sq_count += prob * prob
                        # with gil:
                        #     print(f"Left Feat {c} count: {count_k} prob: {prob} norm: {self.weighted_n_right}")
                    if self.n_classes_sens[s] > 1:
                        interm_gini += (1-sq_count)*(self.n_classes_sens[s]/(2.0*(self.n_classes_sens[s]-1)))  # Normalizes gini from [0,1-1/num_classes]->[0,0.5]
                # Normalize over features
                gini_left += (interm_gini/self.n_sensitive) 
            ...
            for k in range(self.n_outputs):
                ...
                for s in range(self.n_sensitive):   # s iterates over sensitive attributes
                    sq_count = 0.0
                    for c in range(self.n_classes_sens[s]):  # c iterates over the buckets feature s 
                        count_k = self.sum_right_feat_sens[k, s, c]
                        prob = count_k / self.weighted_n_right
                        if c == 0:
                            prob += GAP 
                        else:
                            prob -= GAP
                    ...
                    # with gil:
                    #     if sq_count > 0:
                    #         print(f"sq_count: {1-sq_count} res: {(1-sq_count)*(self.n_classes_sens[s]/(2.0*(self.n_classes_sens[s]-1)))}")
                    if self.n_classes_sens[s] > 1:
                        interm_gini += (1-sq_count)*(self.n_classes_sens[s]/(2.0*(self.n_classes_sens[s]-1)))  # Normalizes gini from [0,1-1/num_classes]->[0,0.5]
                # Normalize over features
                gini_right += (interm_gini/self.n_sensitive) 
            #     with gil:
            #         print(f"Interm-gini: {interm_gini}")
            # with gil:
            #     print(f"My Gini Left: {gini_left} Gini Right: {gini_right}")
            ...
            for k in range(self.n_outputs): # only 1
                sq_count = 0.0
                for c in range(self.n_classes[k]): # basically [0, 1], 2 classes and s sensitives!
                    count_k = self.sum_left[k, c] # n_samples of class c
                    prob = count_k / self.weighted_n_left
                    sq_count += prob * prob

                gini_left += 1.0 - sq_count  # n_samples

            # Right
            for k in range(self.n_outputs): # only 1
                sq_count = 0.0
                for c in range(self.n_classes[k]): # basically [0, 1], 2 classes and s sensitives!
                    count_k = self.sum_right[k, c] # n_samples of class c
                    prob = count_k / self.weighted_n_right
                    sq_count += prob * prob

                gini_right += 1.0 - sq_count  # n_samples

            # with gil:
            #     print(f"Norm Gini Left: {gini_left} Gini Right: {gini_right}")
    ...
    cdef double node_impurity(self) nogil:
        ...
        cdef double res = (1.0 - self.alpha ) * g_label + self.alpha  * (0.5 - g_sens)
        # with gil:
        #     print(f"Res multi-fair {res} - {g_label} - {g_sens}")
        return (1.0 - self.alpha ) * g_label + self.alpha  * (0.5 - g_sens)
