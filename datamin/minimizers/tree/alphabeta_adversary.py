...
class AlphaBetaAdversary:
    ...
    def _confint(self, N: int, x: int, err: float) -> Tuple[float, float]:
        if self.method == "hoeffding":
            ...
            # at max tolerance, add 1e-15 to be safe against fp errors
    ...
    def ub_demographic_parity(self, embeddings: Dict[str, np.ndarray]) -> List[float]:
    ...
    def ub_accuracy(
        self, embeddings: Dict[str, np.ndarray], balanced: bool = False
    ) -> List[float]:
        # Extract data
        ...
        tis = self.bound_val(
            z_val,
            s_val,
            self.err_budget - self.eps_ab - self.eps_glob,
            alpha_ub,
            beta_ub,
        )
    ...
    def bound_train(
        self, z: np.ndarray, s: np.ndarray, budget: float
    ) -> Tuple[float, float]:
        N = z.shape[0]
        pN = (int)((s == 0).sum())
        ...
        if self.verbose:
            print(
                f"[train] budget={budget:.4f} ||| (p={pN/N:.4f}, p_lb={p_lb:.4f}) ==> (alpha={alpha:.4f}, alpha_ub={alpha_ub:.4f}), (q={1 - pN/N:.4f}, q_lb={q_lb:.4f}) ==> (beta={beta:.4f}, beta_ub={beta_ub:.4f})"
            )
    ...
    def bound_val(
        ...
        for (
            ...
            if not BSRCH:
                ...
                q_ub = 1 - p_lb
                # print(f'{alpha_ub * p_ub} AND {beta_ub * q_ub}')
                ti = max(alpha_ub * p_ub, beta_ub * q_ub)
                if ti >= 1:
                    if self.verbose:
                        print(
                            " \n\n(!!!) ti >= 1, this cell should have been skipped to save budget, setting to 1"
                        )
                    ti = 1
            else:
                pi = piNi / Ni
                qi = 1 - pi
                # binary search complicated solution
                lo, hi = max(pi * alpha_ub, qi * beta_ub), max(
                    1 * alpha_ub, 1 * beta_ub
                )

                # Upper bound max(alpha_ub * pi, beta_ub * qi)
                while (hi - lo) > 1e-6:
                    pivot_ti = (lo + hi) / 2
                    # what is error at this threshold?
                    e1, e2 = self._err_at_thresh(
                        Ni,
                        piNi,
                        Ni - piNi,
                        pivot_ti,
    ...
    def bound_test(
        self, z: np.ndarray, s: np.ndarray, budget: float, tis: Dict[int, float]
    ) -> float:
        ...
        if self.verbose:
            print(
                f"[test] budget eps_glob={self.eps_glob:.4f} ||| t0={t0:.4f} (contrib={2*t0:.4f})"
            )
        return T

    def empirical(self, z: np.ndarray, s: np.ndarray) -> float:
        N = z.shape[0]
        pN = (s == 0).sum()
        alpha, beta = 1 / (2 * pN / N), 1 / (2 * (1 - pN / N))
        unique_cells = np.unique(z, axis=0)
