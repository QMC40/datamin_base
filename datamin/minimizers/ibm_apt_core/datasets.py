...
class StoredDataset(Dataset):
    ...
    @staticmethod
    def download(
        url: str, dest_path: str, filename: str, unzip: Optional[bool] = False
    ) -> None:
        """
        ...
        if unzip:
            StoredDataset.extract_archive(
                zip_path=file_path, dest_path=dest_path, remove_archive=False
            )
    ...
    def extract_archive(
        ...
        remove_archive: Optional[bool] = False,
    ):
        """
    ...
    def split_debug(
        ...
        fmt: Optional[Union[str, list]] = None,
    ) -> None:
        """
        ...
        **kwargs,
        ...
        if self.is_pandas:
            if features_names and not np.array_equal(features_names, x.columns):
                raise ValueError(
                    "The supplied features are not the same as in the data features"
                )
