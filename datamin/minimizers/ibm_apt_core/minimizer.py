...
from .datasets import DATA_PANDAS_NUMPY_TYPE, ArrayDataset
...
class GeneralizeToRepresentative(BaseEstimator, MetaEstimatorMixin, TransformerMixin):
    ...
    def __init__(
        ...
        train_only_QI: Optional[bool] = True,
    ...
    def fit(
        self,
        X_train: Optional[DATA_PANDAS_NUMPY_TYPE] = None,
        y_train: Optional[DATA_PANDAS_NUMPY_TYPE] = None,
        X_test: Optional[DATA_PANDAS_NUMPY_TYPE] = None,
        y_test: Optional[DATA_PANDAS_NUMPY_TYPE] = None,
        ...
        dataset = ArrayDataset(
            X_train, y_train, features_names
        )  # feature names are just numbers
        ...
        if (
            ...
            x = pd.DataFrame(dataset.get_samples(), columns=self._features)  # [0..9]
            ...
            if self.train_only_QI:
                used_data = x_QI
                X_train = pd.DataFrame(X_train, columns=self._features)
                X_test = pd.DataFrame(X_test, columns=self._features)
            ...
            for feature in self._features:
                if feature not in feature_data.keys():
                    ...
                    if feature not in self.categorical_features:
                        ...
                        fd["range"] = len(
                            set(values)
                        )  # bug that affects NCP computation
            ...
            categorical_features = [
                ...
                if f in self.categorical_features and f in self.features_to_minimize
            ]
            ...
            numeric_features = [
                f
                for f in self._features
                if f not in self.categorical_features and f in self.features_to_minimize
            ]
            ...
            numeric_features = [
                f for f in self._features if f not in self.categorical_features
            ]
            numeric_transformer = Pipeline(
                steps=[("imputer", SimpleImputer(strategy="constant", fill_value=0))]
            )
            ...
            # tree is built on 1-hot features, so it splits by one possible value always (<=0.5) and not by sets
            T = self.dt_.fit(x_prepared, y_train)

            # manually make used_data (original X) one-hot to get column names presumably to match to x_prepared?
            ...
            # for each leaf gets all ranges for that cell
            self._calculate_cells()

            # does nothing if no categorical
            # for categorical, turns 'ranges' with <=0.5 for categorical into "all other categories"
            self._modify_cells()
            ...
            nodes = self._get_nodes_level(0)  # gets all leaf ids

            # for each cell get row closest to median
            ...
            # basically merge all ranges ever seen
            self._calculate_generalizations()
            print("3. calculated generalizations")
            ...
            x_prepared_test = pd.DataFrame(
                x_prepared_test,
                index=X_test.index,
                columns=self.categorical_data.columns,
            )

            # replace each row with cell representative!
            generalized = self._generalize(
                X_test, x_prepared_test, nodes, self.cells_, self.cells_by_id_
            )
            print("4. generalized")

            # check accuracy
            # a1 = self.estimator.score(ArrayDataset(preprocessor.transform(X_test), y_test))
            # a2 = self.estimator.score(ArrayDataset(preprocessor.transform(x_prepared_test), y_test))
            # print(a1)
            # print(a2)

            accuracy = self.estimator.score(preprocessor.transform(generalized), y_test)
            print(
                "Initial accuracy of model on generalized data "
            ...
            if accuracy > self.target_accuracy:
                ...
                while accuracy > self.target_accuracy:
                    if timeout is not None and time.time() - start_time > timeout:
                        ...
                        print("Time left: %d" % (timeout - (time.time() - start_time)))
                    try:
                        ...
                        accuracy = self.estimator.score(
                            preprocessor.transform(generalized), y_test
                        )
                        # if accuracy passed threshold roll back to previous iteration generalizations
                        print(f"Got {accuracy} and target is {self.target_accuracy}")
                        if accuracy < self.target_accuracy:
                            ...
                            print(
                                "Pruned tree to level: %d, new accuracy: %f"
                                % (level, accuracy)
                            )
                            # print('CELLS:')
                            # for cell in self.cells_:
                            #    print(cell)
                            # print('NEW GENERALIZATIONS:')
                            # print(self.generalizations_)
                            level += 1
            ...
            # if accuracy below threshold, improve accuracy by removing features from generalization
                ...
                while accuracy < self.target_accuracy:
                    ...
                    removed_feature = self._remove_feature_from_generalization(
                        X_test, x_prepared_test, nodes, y_test, feature_data, accuracy
                    )
    ...
    def transform(
        ...
        generalized = pd.DataFrame(
            x, columns=self._features, copy=True
        )  # original data
        ...
        for i in range(len(self.cells_)):
            ...
            for feature in self._features:
                ...
                if feature in self.cells_[i]["representative"] and (
                    ...
                    representatives.loc[i, feature] = self.cells_[i]["representative"][
                        feature
                    ]
        ...
        for f in self._features:
            if f in cell["ranges"]:
                ...
                if not self._cell_contains_categorical(f, cell["categories"][f], x):
                    return False
            elif "untouched" in cell and f in cell["untouched"]:  # bug
                continue
            else:
            ...
            if feature in used_features:
                try:
                    ...
                    X[feature] = pd.Categorical(
                        X.loc[:, feature], categories=values, ordered=False
                    )
    ...
    def _calculate_cells_recursive(self, node):
        ...
        if feature_index == -2:
            ...
            hist = (
                [int(i) for i in self.dt_.tree_.value[node][0]]
        ...
        left_child = self.dt_.tree_.children_left[node]  # feature <= threshold
        right_child = self.dt_.tree_.children_right[node]  # feature > threshold
            ...
            for feature in features:
                if feature in self.oneHotVectorFeaturesToFeatures.keys():
                    ...
                    if categorical_feature not in new_cell["categories"].keys():
                        new_cell["categories"][
                            categorical_feature
                        ] = self.categorical_values[categorical_feature].copy()
                    if feature in cell["ranges"].keys():
                        categorical_value = float(
                            feature[len(categorical_feature) + 1 :]
                        )  # we use floats(?) now, but be careful
            ...
            if nodes:
                for node in nodes:
                    if self.dt_.tree_.feature[node] == -2:  # leaf node
                        ...
                        if "untouched" in left_cell:
                            for feature in left_cell["untouched"]:
                                if (
                                    "untouched" in right_cell
                                    and feature in right_cell["untouched"]
                                ):
    ...
    def _calculate_level_cell_label(self, left_cell, right_cell, new_cell):
        ...
        new_cell["label"] = (
            int(self.dt_.classes_[np.argmax(new_cell["hist"])])
            if not self.is_regression
            else 1
        )
    ...
    def _get_nodes_level(self, level):
        ...
        while len(stack) > 0:
            ...
            if (
                self.dt_.tree_.children_left[node_id]
                != self.dt_.tree_.children_right[node_id]
            ):
    ...
    def _attach_cells_representatives(
        ...
        nodeIds = self._find_sample_nodes(prepared_data, level_nodes)  # find leaf
        ...
        for cell in self.cells_:
            ...
            if self.is_regression:
                ...
                indexes = [
                    i for i, label in enumerate(sample_labels) if label == cell["label"]
                ]
    ...
    def _generalize(
        ...
        )  # empty except for columns
        generalized = pd.DataFrame(
            prepared_data, columns=self.categorical_data.columns, copy=True
        )
        ...
        # iterate over cells (leaves in decision tree)
        print(f"Generalizing over {len(cells)} cells")
    ...
    def _remove_feature_from_generalization(
        ...
        feature = self._get_feature_to_remove(
            original_data, prepared_data, nodes, labels, feature_data, current_accuracy
        )
        if feature is None:
            return None
        ...
        for feature in ranges.keys():
            if feature not in self.generalizations_["untouched"]:
                feature_ncp = self._calc_ncp_numeric(
                    ranges[feature], range_counts[feature], feature_data[feature], total
                )
                if feature_ncp > 0:
                    ...
                    accuracy_gain = (
                        self.estimator.score(
                            self._preprocessor.transform(generalized), labels
                        )
                        - current_accuracy
                    )
                    if accuracy_gain < 0:
    ...
    def _calculate_generalizations(self):
        self.generalizations_ = {
            "ranges": GeneralizeToRepresentative._calculate_ranges(self.cells_),
        ...
        for r in ranges.keys():
            ...
            if not ranges[r]:
                ...
                for value in ranges[r]:
                    ...
                    last_value = value
                counter = [
                    item for item in samples_df[r] if int(item) > last_value
                ]  # bug
                range_counts[r].append(len(counter))
        return range_counts
    ...
    def _calculate_ncp(self, samples, generalizations, feature_data):
        ...
        for feature in ranges.keys():
            ...
            total_features += 1
        for feature in categories.keys():
            featureNCP = self._calc_ncp_categorical(
                categories[feature],
                category_counts[feature],
                feature_data[feature],
                total,
    ...
    def _calc_ncp_categorical(categories, categoryCount, feature_data, total):
        ...
        normalized_category_sizes = [
            s * n / total for s, n in zip(category_sizes, categoryCount)
        ]
    ...
    def _calc_ncp_numeric(feature_range, range_count, feature_data, total):
        # if there are no ranges, feature is supressed (1 big bucket) and NCP is 1 (best)
