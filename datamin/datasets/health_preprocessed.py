...
from datamin.datasets.abstract_dataset import AbstractDataset
...
class HealthDataset(AbstractDataset):
    ...
    def __init__(
        ...
        if not (args.load and path.exists(health_file)):
            ...
            if not path.exists(data_file):
                request.urlretrieve(
                    "https://foreverdata.org/1015/content/HHP_release3.zip", data_file
                )
            ...
            df_claims = self.preprocess_claims(
                pd.read_csv(open(path.join(self.data_dir, "Claims.csv"), "r"), sep=",")
            )
            df_drugs = self.preprocess_drugs(
                pd.read_csv(
                    open(path.join(self.data_dir, "DrugCount.csv"), "r"), sep=","
                )
            )
            df_labs = self.preprocess_labs(
                pd.read_csv(
                    open(path.join(self.data_dir, "LabCount.csv"), "r"), sep=","
                )
            )
            df_members = self.preprocess_members(
                pd.read_csv(open(path.join(self.data_dir, "Members.csv"), "r"), sep=",")
            )

            df_labs_drugs = pd.merge(
                df_labs, df_drugs, on=["MemberID", "Year"], how="outer"
            )
            df_labs_drugs_claims = pd.merge(
                df_labs_drugs, df_claims, on=["MemberID", "Year"], how="outer"
            )
            df_health = pd.merge(
                df_labs_drugs_claims, df_members, on=["MemberID"], how="outer"
            )
        ...
        if args.transfer:
            ...
            drop_cols = [
                ...
                if col.startswith("PrimaryConditionGroup=")
            ]
        ...
        protected = np.logical_or(
            features["AgeAtFirstClaim=60-69"],
            np.logical_or(
        ...
        drop_cols = [
            col for col in df_health.columns if col.startswith("AgeAtFirstClaim=")
        ]
        ...
        for column_name in HealthDataset.column_names:
            ids = [
                ...
                if col.startswith("{}=".format(column_name))
            ]
            if len(ids) > 0:
        ...
        one_hot_keys = [
            k for k in self.one_hot_columns if len(self.one_hot_columns[k]) > 0
        ]
        ...
        selected_column_ids = list(range(features.values.shape[1]))  # All columns
        self.selected_feature_names = [cols[i] for i in selected_column_ids]
        self.selected_cont_features = self.continuous_columns


        if args.fnfpruned:
            self.selected_feature_names = []
            self.selected_cont_features = []
            if args.transfer:
                selected_column_ids = [0, 1, 2, 3, 16, 31, 44]
            else:
                selected_column_ids = [2, 3, 4, 5, 8, 19, 33, 61, 71, 77, 92]
            cols_left = [cols[i] for i in selected_column_ids]

            new_ft_pos: Dict[str, Union[int, Tuple[int, int]]] = {}
            for i, k in enumerate(cols_left):
                new_ft_pos[k] = i
                self.selected_feature_names.append(k)
            for i, j in enumerate(selected_column_ids):
                if j in self.continuous_columns:
                    self.selected_cont_features.append(
                        new_ft_pos[self.selected_feature_names[i]]
                    )
        ...
        if labels.shape[1] == 1:
            labels = labels[:, 0]
        protected = torch.tensor(protected.values.astype(np.bool))  # type: ignore[attr-defined]
        if p_test > 1e-6:
            (
                X_train,
                self.X_test,
    ...
    @staticmethod
    def preprocess_claims(df_claims: pd.DataFrame) -> pd.DataFrame:
        ...
        df_claims = pd.get_dummies(
            df_claims, columns=HealthDataset.claims_cat_names, prefix_sep="="
        )
    ...
    def preprocess_drugs(df_drugs: pd.DataFrame) -> pd.DataFrame:
        ...
        df_drugs["DrugCount"] = df_drugs["DrugCount"].apply(
    ...
    def preprocess_labs(df_labs: pd.DataFrame) -> pd.DataFrame:
        ...
        df_labs["LabCount"] = df_labs["LabCount"].apply(
            lambda x: int(x.replace("+", ""))
        )
        df_labs = (
            df_labs.groupby(["Year", "MemberID"])
            .agg({"LabCount": ["sum", "count"]})
            .reset_index()
        )
...
def load_health(
    ...
    train_dataset = HealthDataset(
        "train", args, p_test=p_test, p_val=0.0, normalize=False
    )
