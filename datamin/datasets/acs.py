...
def load_acs(dataname: str, use_sens: bool) -> Dict[str, Any]:
    toks = dataname.split("-")
    assert len(toks) == 3
    ...
    # We always use 'SEX' as sensitive (1/2), just set 2 to 0
    ...
    if not use_sens:
        ...
        disc_feats = [
            (c - 1 if c > sens_idx else c) for c in disc_feats if c != sens_idx
        ]
    ...
    # return
    data = {
        "train": (X_train_oh, s_train.astype(np.int), y_train.astype(np.int)),  # type: ignore[attr-defined]
        "test": (X_test_oh, s_test.astype(np.int), y_test.astype(np.int)),  # type: ignore[attr-defined]
        "ft_pos": ft_pos,  # important for tree
        "rev_map": inf_cat_rev_mapping,  # Needed for box conc.
    }
    return data
